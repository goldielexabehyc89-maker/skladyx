...
Скрипт: проверяет подтверждение и guard'ы → **снимает бэкап и проверяет его пригодность**
(см. ниже) → `rsync --delete` в `/opt/skladyx` → `docker compose config --quiet` →
`up -d --build` → curl-проверки → `docker compose ps`.

### Backup-гейт: что именно проверяется

Запустить `backup-all.sh` недостаточно — он может завершиться успешно, а дамп окажется
старым или битым. Поэтому перед выкаткой доказывается, что PostgreSQL-дамп создан
**именно этим запуском** и физически читается:

1. перед бэкапом создаётся **маркер времени** (`/tmp/skladyx-deploy-backup-marker.$$`);
2. запускается `backup-all.sh`; ненулевой код → деплой остановлен;
3. дамп ищется как файл **новее маркера** (`find -newer`) — это доказывает, что он от
   этого запуска, а не остался от вчерашнего cron;
4. проверяется, что файл **существует и не пустой** (`test -s`);
5. проверяется **возраст < 10 минут** (`find -mmin -10`);
6. проверяется **читаемость**: `pg_restore --list` разбирает оглавление дампа
   (БД при этом не изменяется), оглавление должно быть непустым.

**Любой сбой на шагах 2–6 останавливает prod-деплой.** Свежий архив uploads выводится
информационно; его отсутствие даёт предупреждение, но не блокирует выкатку
(отказ самого `backup-uploads.sh` уже отловлен на шаге 2).

> Почему маркер, а не «последний файл по времени»: `backup-all.sh` не возвращает путь к
> дампу, а разбирать `backup.log` ненадёжно — в него пишет ещё и cron. Маркер даёт
> проверку на уровне файловой системы и не требует менять backup-скрипты, которые
> используются ночным cron'ом.
