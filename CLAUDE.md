...
> tenant/security-модель, принципы, prod) и [ROADMAP.md](ROADMAP.md) (этапы и доставка).
> Код, тесты, roadmap и старые design-документы не могут переопределять продуктовую спецификацию.
> Новый ответ владельца сначала фиксируется в продуктовой спецификации и только затем реализуется.
>
...
- **Tenant-auth включён:** host резолвится в организацию, неизвестный host закрывается fail-closed,
  а host организации обязан совпадать с `session.companyId`. Доменные запросы дополнительно
  ограничиваются `scoped(session)`.
...
- Роли РостАгро: `ADMIN`, `RECEIVER`, `LOADER`, `PICKER`, `CONTROLLER`, `OBSERVER`; мультироли
  хранятся в `UserRole`, активная рабочая роль и склад — в `WorkShift`. Legacy-роли не расширяют
  права нового workflow.
...
- В каждом задании на бизнес-поведение перечисляй ID правил из
  `docs/ROSTAGRO_PRODUCT_SPEC.md`; неоднозначность не закрывай предположением.
- Если код или тест расходится с продуктовой спецификацией, дефект находится в коде/тесте.
  Остановись и покажи расхождение, не превращай текущее поведение в новое требование.
...
- Бизнес-логику и складские процессы менять только по правилам и ID из
  `docs/ROSTAGRO_PRODUCT_SPEC.md`; текущий legacy-код не является продуктовым требованием.
