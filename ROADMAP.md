...
- [ ] Активация Пакетов 4–5 на prod: создать реальные справочники РостАгро, определить
  порог X и `Warehouse.coolingRate` R, записать настройки защищённой серверной командой и
  только затем включить флаги. Первые операции должны быть реальными, без тестовых
  складских движений.
