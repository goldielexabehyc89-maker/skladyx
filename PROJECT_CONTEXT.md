...
- Утверждённая модель идентичности (Этап 4, проект — docs/MULTITENANT_SECURITY_DESIGN.md):
  глобальный Account (phone/email глобально уникальны, единственный носитель пароля) + per-organization
  Membership (роли, доступ к складам, блокировка). Вход через поддомен организации; сессия отдельная
  на поддомен; авторизация проверяется по членству из БД (немедленный отзыв прав). Реализуется на 4B.
