...
>
> Пакет 9B доставлен на prod: commit `8cf85e8`, без новой миграции. Складские операции
> Пакетов 4–8 переведены на заводские EAN товара; ячейки сканируются по QR/Code 128,
> заказы — по QR. Новые группы не получают внутренний GROUP-QR. Бизнес-процессы
> Пакетов 4–8 по-прежнему не активированы.
>
> Новый интерфейс склада включён на staging и prod:
> `LEGACY_WAREHOUSE_UI_ENABLED=false`, commits `f8f8fb3` и `4309b63`.
> Старые страницы скрыты из навигации и редиректятся в новый интерфейс, но их код и данные
> сохранены для обратимого отката. Номенклатура EAN-центричная: ручное создание —
> наименование + заводской EAN, автоматически LOT/«шт»; API-товары доступны только для чтения.
> Бизнес-процессы Пакетов 4–8 на prod по-прежнему выключены.
